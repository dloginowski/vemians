...
41. **`Test-PRD-P0-56-shop_with_a_door`** — We are a shop with a door before we are a shop with a
    ...
    **REVISED — a working form nobody can find is still a door nobody knows is there.** The
...
30. **`Test-PRD-P0-31-inventory_ledger`** — Stock is a **ledger, not a number**. No tool writes
    ...
    **REVISED: less side padding, once the box no longer needed it.** "Remove some side padding,
    it's wider than it has to be" — with `style_id`'s own box now `6em` wide, its `5px` side
...
71. **`Test-PRD-P0-136-square_custom_attributes`** — The owner's own words, having weighed "ours,
    ...
    **`catalog.set_square_attributes` (T2, manager+) is the mirror image of `catalog.set_channel`
    ...
    category and subcategory was deliberately NOT built at this point: this schema had no subcategory
    concept yet, and inventing the mapping without the owner's own real category/subcategory table
    would have risked assigning genuinely wrong numbers to real inventory — see the REVISED entry at
    the very end of this section, once P0-138 gave subcategories their own numeric_id and the owner
    ...
    **style_id is explicitly NOT the SKU.** The owner's own words, correcting an earlier version of
    ...
    invents the exact FORMAT of. (Both halves of "never assigns automatically" were later revisited,
    separately, on later live feedback: a variation's own `sku` now IS generated by this codebase after
    all, once it turned out Square never does — `Test-PRD-P0-148-auto_generate_variations`'s own REVISED
    entries have the full story — and style_id itself now auto-generates from a category too, the
    REVISED entry at the very end of this section.)
    ...
    **REVISED, once P0-138 gave subcategories their own `numeric_id`: style_id now auto-generates from
    a category, and a conflict auto-bumps instead of refusing.** Two decisions, the owner's own words
    on two separate asks:

    "The style id should auto update from category and subcategory id and an index that auto
    increments. Unless it's provided." `catalog.create_product`, given `category_id` (a SUBCATEGORY —
    one with its own `numeric_id` AND its parent's; `styleIdCodesFor`, `catalog-writer.js`) and no
    `style_id` at all, now builds one from that category's own NN-NN pair plus the next unused index
    under it (`nextStyleIdFor`, scanning `mirror_style_id_ledger` — the permanent record, not just
    `mirror_product`'s own current column, for the same "reserved forever" reason the REVISED entry
    above already established). A bare TOP-LEVEL category (no subcategory chosen) or one with no
    `numeric_id` of its own yet generates nothing — this shop's own `NN-NN-NNN` nomenclature needs both
    halves, and a product filed there still ends up with no style_id, exactly as before this feature
    existed, never a refusal either way. Deliberately creation-time only: an edit
    (`catalog.set_square_attributes`) never invents one from a category on its own — "never generated"
    still holds for every field an edit did not actually name.

    "No it must be auto generated when making the options assignment! ... Auto bump" — the owner's own
    words, the second ask arriving as a correction: an explicit `style_id` that already belongs to
    another product no longer refuses outright in EITHER tool. It bumps to the next free index under
    that same NN-NN prefix instead (`resolveStyleId`, `catalog-write.js`, shared by both tools so they
    cannot disagree about it), and the approval summary says so before anyone approves it — e.g.
    `style_id '01-04-001' is already assigned to 'wool-coat' — used '01-04-002' instead` — never a
