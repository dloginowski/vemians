...
check("test_PRD_P0_108_ops_dashboard__a_dashboard_tab_sits_between_items_and_the_public_site", async () => {
  /* Same-origin, same-iframe-swap tab as Agent/Items — unlike Website
     (cross-origin, its own tab for that reason alone), Dashboard has no
     reason to sit anywhere but the ordinary run of same-origin tabs.
     Superseded from "Messages" (Test-PRD-P0-100-ticket_messaging) once
     the tab stopped being just tickets — see SHELL_TABS' own comment. */
  const { body } = await shell(OWNER);
  const nav = body.match(/<nav class="shell-nav">[\s\S]*?<\/nav>/)[0];
  const buttons = [...nav.matchAll(/<button[^>]*>[^<]*<\/button>/g)].map((m) => m[0]);
  const itemsAt = buttons.findIndex((b) => b.includes('data-src="/items"'));
  const dashboardAt = buttons.findIndex((b) => b.includes('data-src="/dashboard"'));
  const websiteAt = buttons.findIndex((b) => b.includes('data-src="https://vemians.com"'));
  assert.ok(itemsAt > -1 && dashboardAt > -1 && websiteAt > -1, "all three tabs must be present");
  assert.ok(itemsAt < dashboardAt && dashboardAt < websiteAt, "Dashboard sits after Items and before the cross-origin Website tab");
});

check("test_PRD_P0_108_ops_dashboard__tab_equals_dashboard_starts_the_iframe_on_dashboard", async () => {
  const { body } = await shell(OWNER, "/?tab=dashboard");
  assert.match(body, /id="ops-frame" src="\/dashboard"/);
  assert.match(body, /data-src="\/dashboard"[^>]*class="active"/);
});
...
check("test_PRD_P0_124_send_button_active_state__disabled_is_a_dim_orange_not_the_neutral_gray_every_other_send_btn_uses", async () => {
  ...
  const { body } = await frontPage(OWNER);
  assert.match(body, /#chat \.send-btn:disabled, #dash-send:disabled\s*\{[^}]*background:\s*rgba\(217, 119, 87, 0\.35\)/s, "disabled must be a dim tint of the accent hue, not a neutral gray");
  assert.match(body, /#chat \.send-btn:disabled, #dash-send:disabled\s*\{[^}]*opacity:\s*1/s, "full opacity — these colors are already the dim version on purpose, no further fading on top");
});
...
check("test_PRD_P0_126_send_button_disabled_glyph_darker__the_disabled_arrow_uses_the_darker_rule_gray_not_muted", async () => {
  ...
  const { body } = await frontPage(OWNER);
  assert.doesNotMatch(body, /#chat \.send-btn:disabled, #dash-send:disabled\s*\{[^}]*color:\s*var\(--muted\)/s, "the old, too-light --muted glyph must be gone");
});
...
check("test_PRD_P0_127_send_button_disabled_glyph_matches_mic__the_disabled_arrow_matches_the_mic_buttons_own_dark_icon", async () => {
  ...
  assert.match(body, /#chat \.send-btn:disabled, #dash-send:disabled\s*\{[^}]*color:\s*var\(--ground\)/s, "the disabled glyph must match the mic button's own dark icon color exactly");
  assert.doesNotMatch(body, /#chat \.send-btn:disabled, #dash-send:disabled\s*\{[^}]*color:\s*var\(--rule\)/s, "the previous round's --rule attempt must be gone");
});
...
check("test_PRD_P0_124_send_button_active_state__the_active_glyph_is_bright_not_the_near_black_ground_token", async () => {
  /* The owner's own words: "when it's active, it's a much brighter
     ...
     bright warm-white. */
  const { body } = await frontPage(OWNER);
  assert.match(body, /#chat \.send-btn, #dash-send \{\s*background:\s*var\(--accent\); color:\s*var\(--ink\);\s*\}/, "the active (enabled) icon must be the bright --ink token, not --ground");
  assert.doesNotMatch(body, /#chat \.send-btn, #dash-send \{\s*background:\s*var\(--accent\); color:\s*var\(--ground\);\s*\}/, "the old near-black --ground icon must be gone");
});
